...
if __name__ == "__main__":
    ...
    # Apply contrast
    contrasted_bytes = preprocessor.adjust_contrast(original_bytes, 0.1)

    # Apply brighten only, with no contrast
    brighten_bytes = preprocessor.brighten(original_bytes, 5)
    
    # Apply brighten + contrast
    processed_bytes = preprocessor.brighten(contrasted_bytes, 0.1)
    
    contrasted_bytes_direct = cv2.convertScaleAbs(original_bytes, alpha=1.5, beta=0)
    brighten_bytes_direct = cv2.convertScaleAbs(original_bytes, alpha=1, beta=1)
    ...
    with open("test_contrasted_direct.jpg", "wb") as f:
        f.write(contrasted_bytes_direct)
    with open("test_brighten_direct.jpg", "wb") as f:
        f.write(brighten_bytes_direct)
