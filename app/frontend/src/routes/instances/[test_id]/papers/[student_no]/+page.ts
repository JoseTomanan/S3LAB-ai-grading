...
export const load: PageLoad = async ({ fetch, params, parent }) => {
  ...
  try {
    student_items = await api<(StudentAnswer & {label: string})[]>(
      `${API_URL}/api/student_answers/${params.test_id}/${params.student_no}`,
      undefined,
      fetch
    ) ?? [];
  } catch (e) {
  ...
  try {
    const result = await api<{ evaluations: GetSpecificEvaluationResponse[] }>(
      `${API_URL}/api/student_answers/${params.test_id}/results/${params.student_no}`,
      undefined,
      fetch
    );
